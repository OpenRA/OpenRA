	...
	public class WorldInteractionControllerWidget : Widget
	{
			...
			if (mi.Button == MouseButton.Left && mi.Event == MouseInputEvent.Up)
			{
				if (World.OrderGenerator is UnitOrderGenerator)
				{
					if (useClassicMouseStyle && HasMouseFocus)
					{
						...
						{
							if (!(World.ScreenMap.ActorsAt(xy).Where(x => x.Info.TraitInfosAny<SelectableInfo>() &&
		...
		static IEnumerable<Actor> SelectActorsInBoxWithDeadzone(World world, int2 a, int2 b)
		{
			...
			return world.ScreenMap.ActorsInBox(a, b)
				.Where(x => x.Info.TraitInfosAny<SelectableInfo>() && (x.Owner.IsAlliedWith(world.RenderPlayer) || !world.FogObscures(x)))
