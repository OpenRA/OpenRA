	...
	public static class UnitOrders
	{
		...
		internal static void ProcessOrder(OrderManager orderManager, World world, int clientId, Order order)
		{
			switch (order.OrderString)
			{
				// Server message
				case "Message":
					TextManager.AddSystemLine(order.TargetString);
					break;
				...
				case "Chat":
					{
						...
						if (order.ExtraData == 0)
						{
							...
							TextManager.AddChatLine(client.Name + suffix, client.Color, message);
							break;
						}
						...
						if (world == null)
						{
							...
							if (orderManager.LocalClient != null && client.Team == orderManager.LocalClient.Team)
								TextManager.AddChatLine(prefix + client.Name, client.Color, message);
						...
						if (order.ExtraData == uint.MaxValue && localClientIsObserver)
						{
							...
							if (client.IsObserver || (player != null && player.WinState != WinState.Undefined))
								TextManager.AddChatLine("[Spectators] " + client.Name, client.Color, message);
						...
						if (valid && (isSameTeam || world.IsReplay))
							TextManager.AddChatLine("[Team" + (world.IsReplay ? " " + order.ExtraData : "") + "] " + client.Name, client.Color, message);
				...
				case "StartGame":
					{
						...
						if (!string.IsNullOrEmpty(order.TargetString))
						{
							...
							TextManager.AddSystemLine("The game has started.");
				...
				case "GameSaved":
					if (!orderManager.World.IsReplay)
						TextManager.AddSystemLine("Game saved");
				...
				case "PauseGame":
					{
						...
						if (client != null)
						{
							...
							if (orderManager.World.Paused != pause && world != null && world.LobbyInfo.NonBotClients.Count() > 1)
							{
								...
								TextManager.AddSystemLine(pausetext);
							}
