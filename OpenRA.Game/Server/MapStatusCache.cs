...
namespace OpenRA.Server
{
	...
	public class MapStatusCache
	{
		...
		void RunLintTests(MapPreview map, Ruleset rules)
		{
			...
			{
				Log.Write("server", $"Map {(map as IMapCredentials)?.Title} failed lint with error: {message}");
				failed = true;
			}
		...
		public Session.MapStatus this[MapPreview map]
		{
			get
			{
				...
				{
					Log.Write("server", $"Failed to load rules for `{(map as IMapCredentials)?.Title}` with error :{e.Message}");
					status = Session.MapStatus.Incompatible;
				}

				if (map.Players.Players.Count > MapPlayers.MaximumPlayerCount)
				{
					Log.Write("server", $"Failed to load `{(map as IMapCredentials)?.Title}`: Player count exceeds maximum ({map.Players.Players.Count}/{MapPlayers.MaximumPlayerCount}).");
					status = Session.MapStatus.Incompatible;
				}
