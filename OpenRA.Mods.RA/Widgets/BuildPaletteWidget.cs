...
//			if ((e.Event == KeyInputEvent.Up) || (e.Modifiers.HasModifier(Modifiers.Ctrl))) return false;
			if (e.KeyName == "tab")
			{
		...
		bool DoBuildingHotkey(string key, World world, KeyInput e)
		{
			...
			if ( (toBuild != null) && (!toBuild.Traits.Contains<ProductionBuildingInfo>() ^ e.Modifiers.HasModifier(Modifiers.Ctrl)))
			{
				Sound.Play(TabClick);
