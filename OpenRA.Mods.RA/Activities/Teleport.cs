...
namespace OpenRA.Mods.RA.Activities
{
	public class Teleport : Activity
	{
		...
		{
			if (maximumDistance != null && maximumDistance > maxCellSearchRange)
				throw new Exception("Teleport cannot be used with a maximum teleport distance greater than {0}.".F(maxCellSearchRange));
		...
		CPos? ChooseBestDestinationCell(Actor self, CPos destination)
		{
			...
			{
				var closest = restrictTo.OrderBy(x => (x - destination).Length).FirstOrDefault();
			...
			for (int r = 1; r <= maxCellSearchRange || (maximumDistance != null && r <= maximumDistance); r++)
			{
				...
				{
					if (!self.Owner.Shroud.IsExplored(tile) || (restrictTo != null && !restrictTo.Contains(tile)))
						searched.Add(tile);

					if (pos.CanEnterCell(tile))
						return tile;
				}
			}
