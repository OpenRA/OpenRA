...
{
	public class MissileASInfo : IProjectileInfo
	{
		...
		public IProjectile Create(ProjectileArgs args) { return new MissileAS(this, args); }
	}

	// TODO: double check square roots!!!
	public class MissileAS : IProjectile, ISync
	{
		enum States
		{
			Freefall,
		...
		public MissileAS(MissileASInfo info, ProjectileArgs args)
		{
			this.info = info;
			this.args = args;
		...
		public void Tick(World world)
		{
			...
			// Move the missile
			// Graion: Allow the missile to strap on the target in range - this might look jarring but prevents the circling-missile-bug
			var lastPos = pos;
			if (state != States.Freefall && relTarDist < move.Length)
