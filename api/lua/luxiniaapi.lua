...
window = {type='class',description="The graphics window luxinia runs in. Setters only have effect after update is called. Do not call setters without ever calling update.",childs={["refsize"] = {type='function', description = " sets reference size of screen (default=640,480). Reference size is mostly important for list2d rendering and mouse positions. It allows the window not use pixel positions but custom dimensions which gui will be stretched to.", args="([int w,h])", returns="([int w,h])"},
...
["getdata"] = {type='function', description = " if texture was not loaded with keepdata, we can retrieve imagedata this way. Also allows updating local data from vidmem if keepdata was set on init, but texture content has changed. Note that without keepdata set, you must refetch data everytime major visual changes occur (fullscreen/res toggle).", args="(texture)", returns="()"},
...
["datapointer"] = {type='function', description = " if the texture was loaded with keepdata you can access the memory directly in other lua dlls. Be aware that you must make sure to not corrupt memory, make sure you are always smaller than the 'end' pointer.", args="(texture)", returns="(pointer start, end)"},
...
rcmdcopytex = {type='class',description="Copys drawbuffer to texture",childs={["sideordepth"] = {type='function', description = " returns or sets side of texture for cubemaps 0..5, or z slice depth for 3d textures.", args="(rcmdcopytex,[int])", returns="([int)"},
...
["offset"] = {type='function', description = " returns or sets offset into texture.", args="(rcmdcopytex,[int x,y])", returns="([int x,y])"},
