...
["new"] = {type='function', description = " creates a trimesh geom, ready for collision.", args="(dgeomtrimeshdata,[dspace space])", returns="(dgeomtrimesh)"}}}
...
["simplerenderqueue"] = {type='function', description = " sets up a standard renderqueue for the view (which is also returned).\
	The l3dview has equivalent rcmds in its table. The order is: rClear,rDrawbg,rLayers[1..16].\
...
window = {type='class',description="The graphics window luxinia runs in. Setters only have effect after update is called. Do not call setters without ever calling update.",childs={["fullscreen"] = {type='function', description = " gets or sets if window will be fullscreen", args="([boolean])", returns="([boolean])"},
...
["bpp"] = {type='function', description = " returns or sets bits per pixel (16,24,32)", args="([int])", returns="(int)"},
...
["load"] = {type='function', description = " loads a shader", args="(string filename)", returns="(shader shd)"},
...
gpuprog = {type='class',description="It contains code to be executed on the graphics card. Depending on whether your hardware is capable or not shaders will load gpuprogs automatically. There is als a set of standard gpuprograms in the base/gpuprogs folder of the application which are used internally to speed rendering up.",childs={["find"] = {type='function', description = " finds a loaded gpuprog", args="(string file,string entryname)", returns="([gpuprog])"},
["getrestype"] = {type='function', description = " returns the resource type as int value, useful for resdata or reschunk functions", args="()", returns="(int restype)"},
["source"] = {type='function', description = " returns the source", args="(gpuprog)", returns="(string)"},
...
testsys = {type='class',description="The testsys class is for testing purpose such as testing 		implementations during development of the luxinia core. 		It is not meant to be published in the release versions of Luxinia.",childs={["userdata"] = {type='function', description = " if no argument was passed, it returns 		a userdata value. If that userdata value is passed, it will return true if 		it was read successfully. It returns false if the passed value was incompatible.", args="([userdata])", returns="([userdata/boolean,int])"}}}
renderfbo = {type='class',description="The framebufferobject (fbo) can be used for advanced off-screen rendering effects. For this class you need special capability. When not using fbos rendering is done to the windows's backbuffer. The 'fbo' rcmds can be used to make use of it.",childs={["size"] = {type='function', description = " returns or sets fbo size. All attachments must have equal size.", args="(renderfbo,[int width,height,[boolean windowsized ]])", returns="([int width,height, boolean windowsized])"},
...
ubytearray = {type='class',description="Unsigned byte array in Luxinia for array operations.",childs={["sub"] = {type='function', description = " self-=a or self = a - b", args="(ubytearray self,ubytearray / value a, [ubytearray b])", returns="()"},
["set"] = {type='function', description = " self = a (copy an array or set value to all)", args="(ubytearray self,ubytearray / value a)", returns="()"},
["max"] = {type='function', description = " self = max(self,a) or self = max(a,b)", args="(ubytearray self,ubytearray / value a, [ubytearray b])", returns="()"},
["div"] = {type='function', description = " self/=a or self = a / b", args="(ubytearray self,ubytearray / value a, [ubytearray b])", returns="()"},
["fromushort"] = {type='function', description = " copies content to ubytearray. Min will become 0 and max 255", args="(ubytearray,ushortarray,int min,max)", returns="()"},
["min"] = {type='function', description = " self = min(self,a) or self = min(a,b)", args="(ubytearray self,ubytearray / value a, [ubytearray b])", returns="()"},
["mul"] = {type='function', description = " self*=a or self = a * b", args="(ubytearray self,ubytearray / value a, [ubytearray b])", returns="()"},
["vector4"] = {type='function', description = " returns or sets ith vector4, make sure count is 4*maxindex.", args="(ubytearray,int index, [int x,y,z,w])", returns="([int x,y,z,w])"},
["vector3"] = {type='function', description = " returns or sets ith vector3, make sure count is 3*maxindex.", args="(ubytearray,int index, [int x,y,z])", returns="([int x,y,z])"},
["vector3all"] = {type='function', description = " returns or sets all values of the ubytearray, optionally can set from which int to start and how many vectors. Vectorsize is 3+stride.", args="(ubytearray,int x,y,z,[int startubyte, vectorcount, stride])", returns="()"},
["add"] = {type='function', description = " self+=a or self = a + b", args="(ubytearray self,ubytearray / value a, [ubytearray b])", returns="()"},
["new"] = {type='function', description = " creates a new staticarray. Count must be >0.", args="(int count)", returns="(ubytearray)"}}}
...
["arrayset"] = {type='function', description = " sets pixels from floatarry (4 floats per pixel). Optionally you can mask off certain channels.", args="(texture, floatarray, int x,y,w,h,[boolean mask r,g,b,a])", returns="()"},
["create2drect"] = {type='function', description = " creates a new empty rectangle texture in memory. Width and height can be arbitrary. texdatatype is ubyte (Unsigned Byte) by default. Automatically registered as USER_TEX. No compression supported.", args="(string name, int width, int height,textype, [boolean keepdata], [boolean nofilter],[texdatatype])", returns="(texture)"},
...
["reload"] = {type='value', description = "() : (texture,[string filename,boolean keepoldsize] - reloads the texture with original (or new filename) with same specifications as on first load. The internal filename remains to be the original filename. Optionally the texture can be forced to the current texturesize."},
...
["convfilterboxdata"] = {type='function', description = " runs convolution boxcar filter on the imagedata. wrap defaults to true", args="(texture,int kernelsize,[boolean wrap])", returns="()"},
...
["arraydecal"] = {type='function', description = " decal blends the floatarray (4 floats per pixel) on top of current imagedata rectangle, defined by x,y,w,h. Optionally you can mask off certain channels.", args="(texture, floatarray, int x,y,w,h,[float alphamul],[boolean mask r,g,b,a])", returns="()"},
...
["convfiltergaussdata"] = {type='function', description = " runs convolution gauss filter on the imagedata. wrap defaults to true", args="(texture,int kernelsize,[boolean wrap])", returns="()"},
...
["resize"] = {type='function', description = " resizing textures. previous data is lost.", args="(texture,int width, int height)", returns="()"},
...
["createcube"] = {type='function', description = " creates a new empty texture in memory. Size must be power of two. Automatically registered as USER_TEX. Same creation, compression and mipmap rules as for new2d", args="(string name, int size, textype, [boolean keepdata], [boolean nofilter],[boolean mipmap],[texdatatype])", returns="(texture)"},
...
["deletedata"] = {type='function', description = " deletes the imagedata in system memory. you can now longer change or sample it.", args="(texture)", returns="()"},
...
["getdata"] = {type='function', description = " if texture was not loaded with keepdata, we can retrieve imagedata this way and set keepdata", args="(texture)", returns="()"},
["datapointer"] = {type='function', description = " if the texture was loaded with keepdata you can access the memory directly in other lua dlls. Be aware that you must make sure to not corrupt memory, make sure you are always smaller than the 'end' pointer.", args="(texture)", returns="(int start, int end)"},
...
["datastring"] = {type='function', description = " returns the local data as binary lua string. Type can be 0 = full data as is. 1 = RGB color array (only allowed for uncompressed unsigned byte), luminance values will be expanded. 2 = RGB and split ALPHA string (for 2 or 4 component colors, others will revert to case 1).", args="([int type])", returns="(string,[string])"},
["formattype"] = {type='function', description = " the format in which the texture is stored as string, e.g. 1d, 2d, 3d, rect or cube.", args="(texture)", returns="(string)"},
...
\
A pattern cannot contain embedded zeros. Use %z instead. ",childs={["double2bin"] = {type='function', description = " converts number in 8 chars with double precision (64bit)", args="(number)", returns="(string)"},
["char2bin"] = {type='function', description = " converts number in single char for numbers between -127 and 128 - rounded, no overflowcheck done", args="(number)", returns="(string)"},
["int2bin"] = {type='function', description = " converts number in 4 chars for numbers between -2147483647 and 2147483647 - rounded, no overflowcheck done", args="(number)", returns="(string)"},
["bin2float"] = {type='function', description = " converts 4 chars in a floating point number with float precisision", args="(string)", returns="(number)"},
["match"] = {type='function', description = "\
...
["bin2short"] = {type='function', description = " converts 2 chars in a number between -32767 and 32768", args="(string)", returns="(number)"},
["dumpstripped"] = {type='function', description = "\
Returns a string containing a binary representation of the given function,\
so that a later loadstring on this string returns a copy of the function.\
\
Any debug information is stripped out here. ", args="(function f)", returns="(string binarybytecode)"},
...
["short2bin"] = {type='function', description = " converts number in 2 chars for numbers between -32767 and 32768 - rounded, no overflowcheck done", args="(number)", returns="(string)"},
["bin2double"] = {type='function', description = " converts 8 chars in a floating point number with double precisision", args="(string)", returns="(number)"},
...
["float2bin"] = {type='function', description = " converts number in 4 chars with float precision (32 bit)", args="(number)", returns="(string)"},
["bin2char"] = {type='function', description = " converts single char in a number between -127 and 128", args="(string)", returns="(number)"},
["reverse"] = {type='function', description = " Returns a string that is the\
				string s reversed.", args="(string s)", returns="(string reversed)"},
["bin2int"] = {type='function', description = " converts 4 chars in a number between -2147483647 and 2147483648", args="(string)", returns="(number)"},
...
dcollider = {type='class',description="Subclasses can perform collision detection.",childs={["test"] = {type='function', description = " 			   tests two dcolliders against each other and returns the number of contacts, the position coordinates 			   as a floatarray (x,y,z,...), the normals as a floatarray, the depth as a floatarray (d1,d2,...), 			   the contactgeoms as a table and triangle data as floatarry (index,u,v,...).\
\
...
\
			   If no string is passed, the returned values are as shown above, otherwise the 			   appropriate arrays are returned in the given order specified by the passed string. 			   If the string contains other characters, an error is thrown. Triangle information is only accurate when testing with rays and 			   raytrisresult-enabled dgeomtrimeshes, else the triangle index is -1", args="(dcollider,dcollider, [string whichresults])", returns="(int count,[table pos, table normal, table depth, table geom, table tri])"},
["delete"] = {type='function', description = " deletes a dcollider", args="(dcollider)", returns="()"},
["testnear"] = {type='function', description = "				   tests two geoms if they are near each other (aaboxes overlap).", args="(dcollider a, dcollider b, int maxcount)", returns="(int count,table geomi])"},
["testtriarray"] = {type='function', description = "					   tests two geoms for intersecting triangles. Only works if triarrayresult is activated for the included trimeshgeoms. 					   Each collision pair results into trigeom/geom pair in geomi table. Associated with startindex/numberoftris pair tristart. Startindex maps as offset index into triall, which then has numberoftris*3 int indices.", args="(dcollider a, dcollider b, int maxcount)", returns="(int count,table geomi,table tristart,table triall])"}}}
terraindecal = {type='class',description="A decalsurface on top of a terrain.",childs={["rotdeg"] = {type='function', description = " rotation in degrees", args="(terraindecal,[float])", returns="([float])"},
["delete"] = {type='function', description = " deletes decal", args="(terraindecal)", returns="()"},
["blendmode"] = {type='function', description = " blendmode", args="(terraindecal,[blendmode])", returns="([blendmode])"},
["deactivate"] = {type='function', description = " deactivates decal", args="(terraindecal)", returns="()"},
["matsurface"] = {type='function', description = " materal/texture of decal", args="(terraindecal,[matsurface])", returns="([matsurface])"},
["activate"] = {type='function', description = " activates decal on given terrain. optional lifetime", args="(terraindecal,tbterrain,[int lifetime])", returns="()"},
["rotrad"] = {type='function', description = " rotation in radians", args="(terraindecal,[float])", returns="([float])"},
["size"] = {type='function', description = " size", args="(terraindecal,[float])", returns="([float])"},
["pos"] = {type='function', description = " center of decal", args="(terraindecal,[float x,y])", returns="([float x,y])"},
["new"] = {type='function', description = " new decal", args="()", returns="(terraindecal)"}}}
capability = {type='class',description="Certain system (hardware related) capabilites can be queried for support here.",childs={["texcomb_combine4"] = {type='function', description = " returns support for combine4 in texcombiners", args="()", returns="(boolean)"},
...
["dot3normalmap"] = {type='function', description = " returns support for VID_NORMAL combiner in shader system.", args="()", returns="(boolean)"},
...
soundlistener = {type='class',description="The soundlistener represents the position, velocity and orientation 		of the 'microphone' in space.",childs={["orientation"] = {type='function', description = " sets/gets orientation of listener", args="([float x,y,z,upx,upy,upz])", returns="([float x,y,z,upx,upy,upz])"},
["vel"] = {type='function', description = " sets or gets velocity of listener", args="([float x,y,z])", returns="([float x,y,z])"},
...
simplespace = {type='class',description="a simple sphere collisionspace",childs={["hitlist"] = {type='function', description = " returns first element of hitlists", args="(simplespace self)", returns="([sphere shape]/[aabox shape])"},
["list"] = {type='function', description = " returns first element of list", args="(simplespace self)", returns="([sphere shape]/[aabox shape])"},
["test"] = {type='function', description = " tests for collision with sphere", args="(simplespace self, float x,y,z,[r=0])", returns="(int hitcount)"},
["add"] = {type='function', description = " adds a shape to the space", args="(simplespace self,shape shape)", returns="()"},
["new"] = {type='function', description = " constructs an empty simplespace", args="()", returns="(SimpleSpace space)"}}}
...
["new"] = {type='function', description = " returns a new l3dview.", args="()", returns="(l3dview)"},
...
["bgskyl3dnode"] = {type='function', description = " returns or sets l3dnode to be used as background, disabled when 2nd arg is not a l3dnode. Be aware that the l3dnode must not be used anywhere else. It will automatically set the manual world matrix mode. Camera position is scaled using the 3 floats, that way you can 'fix' the model position around camera. Be aware that background models write into max depth, which means that overlapping geometry might create issues.", args="(l3dview,[l3dnode],[float x,y,z])", returns="([l3dnode])"},
...
["bgmatsurface"] = {type='function', description = " returns or sets matsurface, disabled when 2nd arg is not a matsurface", args="(l3dview,[matsurface])", returns="([matsurface])"},
...
["trianglefan"] = {type='function', description = " triangle fan. First index becomes center, all others are connected to it and previous index.", args="()", returns="(primitivetype)"},
["triangles"] = {type='function', description = " triangle list. Every 3 indices make a triangle.", args="()", returns="(primitivetype)"}}}
l3dtbterrain = {type='class',description="list3d node of a tbterrain",childs={["getterrain"] = {type='function', description = " gets terrain of this l3dnode", args="(l3dnode node)", returns="(tbterrain)"},
["new"] = {type='function', description = " new l3dnode of tbterrain", args="(string name,|l3dlayerid|, tbterrain)", returns="(l3dterrain)"}}}
...
["tpPrev"] = {type='function', description = " gets previous trailpoint (or null).", args="(l3dtrail,trailpoint)", returns="([trailpoint])"},
...
["tpTime"] = {type='function', description = " gets or sets trailpoint's spawntime in ms.", args="(l3dtrail,trailpoint,[int])", returns="([int])"},
...
["tpNext"] = {type='function', description = " gets next trailpoint (or null).", args="(l3dtrail,trailpoint)", returns="([trailpoint])"},
...
["lockMouse"] = {type='function', description = " locks the mouse to this component. As long as the\
	...
	does not affect MouseListeners that are attached to the MouseCursor.", args="(Component)", returns="()"},
...
["rsBlendinvert"] = {type='function', description = " returns or sets blend invert of alpha-based blend types", args="(rendersurface,[boolean])", returns="([boolean])"},
...
simplecollision = {type='class',description="A very basic collision system just allowing sphere to sphere checks and not very optimized",childs={}}
...
["new"] = {type='function', description = " returns the rcmd", args="()", returns="(rcmddrawl2d)"}}}
...
",childs={}}
GuiHelpers = {type='class',description="GuiHelpers contain a few functions to aid application development.\
	",childs={["scrollfield"] = {type='function', description = "\
...
["addcolorbutton"] = {type='function', description = "\
	...
	Functions: setcolor(table colorRGBA, [boolean noevent]<br>\
	Parameters table contains:<br>\
	...
	* [cancel]: string, cancel text\
	", args="(Container frame,int x,y,w,h, string title, table parameters)", returns="(int newy, fn setcolor)"},
["addslider"] = {type='function', description = "\
	creates a slider with leading description label and following value label.tion. New y value which is y + h.<br>\
...
["frameminmax"] = {type='function', description = "\
...
["new"] = {type='function', description = " constructs identity matrix or uses lua table to fill values. If a table 		with 16 elements is passed, the values are interpreted as a matrix and will 		The OpenGL matrixstyle (column-major) is used so the x,y,z translation is found at element index 12,13,14 (or 13,14,15 as lua arrays start at 		index 1, while C arrays start at index 0)", args="([table float16])", returns="(matrix4x4 m)"},
...
["fbotest"] = {type='function', description = " performs a test run on current rcmdfbo related setups. It takes enable flags of rcmds into account. Returns an error (+1) or warning(-1) string, or nothing if all is working.", args="()", returns="([int stringtype, string message])"}}}
...
["removeChilds"] = {type='function', description = " removes all childs of the container", args="(Container)", returns="()"},
...
rcmdstencil = {type='class',description="Sets stencil environment.",childs={["new"] = {type='function', description = " returns the rcmd", args="()", returns="(rcmdstencil)"}}}
...
texdatatype = {type='class',description="For creating textures this defines the internal storage precision. Some may require special capabilities",childs={["ushort"] = {type='function', description = " unsigned byte. normalized 0-1. 16-bit precision, local data storage as unsigned short.", args="()", returns="(texdatatype)"},
["ubyte"] = {type='function', description = " unsigned byte. normalized 0-1. 8-bit precision, local data storage as unsigned byte.", args="()", returns="(texdatatype)"},
...
["float32"] = {type='function', description = " float32. arbitrary, 32-bit precision, local data storage as float32.", args="()", returns="(texdatatype)"}}}
...
technique = {type='class',description="Techniques are used in shaders and depend on the graphics hardware capabilities.",childs={["arb_texcomb"] = {type='function', description = " arbitrary texture access in combiners, cubemap textures, dot3 combiner.", args="()", returns="(technique)"},
...
["cg_sm3_tex8"] = {type='function', description = " advanced cg support, shader model 3.0 and 8 texturecoords", args="()", returns="(technique)"},
["lowdetail"] = {type='function', description = " lowdetail, overrides all other techniques", args="()", returns="(technique)"},
["arb_vf_tex8"] = {type='function', description = " same as arb_vf with 8 texturecoords", args="()", returns="(technique)"},
...
trailpoint = {type='class',description="A point part of a l3dtrail",childs={}}
...
blendmode = {type='class',description="They define how texels or pixels are blend with the destination. Sometimes you might be able to use inverte d blends, those just invert the self.alpha value",childs={["decal"] = {type='function', description = " out = destination * (1-self.alpha) + self * (self.alpha)", args="()", returns="(blendmode)"},
...
meshid = {type='class',description="Mesh within a model. Typically contains vertices and indices, as well as material information.",childs={["boneid"] = {type='function', description = " returns or sets boneid it is linked to. Only makes sense if model is animateable).", args="(meshid,[boneid])", returns="([boneid])"},
["matsurface"] = {type='function', description = " returns matsurface", args="(meshid)", returns="([matsurface])"},
["name"] = {type='function', description = " returns or sets name of mesh", args="(meshid)", returns="([string])"},
["init"] = {type='function', description = " only works on empty meshes that are hosted in model.create generated model. Fill with content via indexarray and vertexarray interfaces (don't forget to set indexCount and vertexCount and at the end indexMinmax!).", args="(meshid,string name, int vertexcount, int indicescount, [texture/material name])", returns="()"},
...
["vertexUpdateVBO"] = {type='function', description = " if possible resubmits array using vertexAllocatedcount to VBO, returns true on success else false (no VBO or non compatible vertexarray).", args="(vertexarray)", returns="(boolean)"},
...
["vertexUser4"] = {type='function', description = " set or return user4 vector, only bigvertex vertexarrays will allow this. It overwrites texcoord channel 2 and 3.", args="(vertexarray, int index,[float x,y,z,w])", returns="([float x,y,z,w])"},
...
["vertexNormal"] = {type='function', description = " set or return normal, some vertexarrays do not have normals", args="(vertexarray, int index,[float x,y,z])", returns="([float x,y,z])"},
...
["vertexPtrs"] = {type='function', description = " returns C pointers for begin and end of array.", args="(vertexarray)", returns="(int start,end])"},
...
["cgallowglslatism3"] = {type='function', description = " gets or sets if Cg on startup should allow use of GLSL for ATI shader model3 capable cards. As Cg's GLSL support remains to have issues this defaults to false.", args="([boolean])", returns="([boolean])"},
...
ushortarray = {type='class',description="Unsigned short array in Luxinia for array operations.",childs={["fromushort"] = {type='function', description = " copies content to ushortarray. Min will become 0 and max 65535", args="(ushortarray,ushortarray,int min,max)", returns="()"},
["new"] = {type='function', description = " creates a new staticarray. Count must be >0.", args="(int count)", returns="(ushortarray)"}}}
...
sphere = {type='class',description="a simple sphere",childs={["prevhit"] = {type='function', description = " prev element in hitlist (ring)", args="(sphere self)", returns="(shpere / aabox shape)"},
["r"] = {type='function', description = " radius of sphere", args="(sphere self, [float r])", returns="([float r])"},
["nexthit"] = {type='function', description = " next element in hitlist (ring)", args="(sphere self)", returns="(shpere / aabox shape)"},
["remove"] = {type='function', description = " removes the sphere from his space", args="(sphere self)", returns="()"},
["pos"] = {type='function', description = " center of sphere", args="(sphere self, [float x,y,z])", returns="([float x,y,z])"},
["prevlist"] = {type='function', description = " prev element in spacelist (ring)", args="(sphere self)", returns="(shpere / aabox shape)"},
["space"] = {type='function', description = " returns his space, if it is in a space", args="(sphere self)", returns="([Space space])"},
["nextlist"] = {type='function', description = " next element in spacelist (ring)", args="(sphere self)", returns="(shpere / aabox shape)"},
["id"] = {type='function', description = " id of sphere", args="(sphere self, [int id])", returns="([int id])"},
["new"] = {type='function', description = " constructs a simple sphere", args="([float x,y,z]/[float r],[float r],[int id])", returns="(Sphere sphere)"}}}
...
["getcomment"] = {type='function', description = " reads the comments of a musicfile and returns a table that contains a list of it.", args="(string file)", returns="([table info])"},
...
tls = {type='class',description="TLS stands for 'threaded lua state' and is\
...
\
	...
	",childs={["xlock"] = {type='function', description = " locks the exchange state until it is unlocked. The\
	...
	state.\
", args="(tls)", returns="()"},
["xunlock"] = {type='function', description = " unlocks the exchange state\
", args="(tls)", returns="()"},
["running"] = {type='function', description = " returns true if the thread is still running\
", args="(tls)", returns="(boolean isrunning)"},
["stop"] = {type='function', description = " destroys the thread, but waits until the mutex was\
	unlocked. Don't use this function unless really required, as\
	it might damage the luastate which can lead to a crash once\
	it is reused.\
", args="(tls)", returns="()"},
["xget"] = {type='function', description = " returns a\
	value for the key from the global environment table of the exchangestate\
", args="(tls, key)", returns="(boolean value / [nil, typename])"},
["start"] = {type='function', description = "\
...
\
\
", args="(tls,string code)", returns="([boolean success],[string errormsg])"},
["xset"] = {type='function', description = " sets a\
	value for the key in the global environment table of the exchangestate\
", args="(tls, key,value)", returns="([nil, error])"},
["xdostring"] = {type='function', description = " executes a piece of\
...
", args="(string code)", returns="([true,... / nil, error])"},
["new"] = {type='function', description = " creates a new threaded lua state object.\
", args="()", returns="(tls)"}}}
...
soundnode = {type='class',description="soundnodes are sound sources in space.",childs={["pitch"] = {type='function', description = " sets or gets pitch value", args="(soundnode self, [float])", returns="([float])"},
["delete"] = {type='function', description = " deletes soundnode", args="(soundnode self)", returns="()"},
["rewind"] = {type='function', description = " rewinds soundnode", args="(soundnode self)", returns="()"},
...
["new"] = {type='function', description = " creates a new transformobject", args="([dspace space])", returns="(dgeomtranform)"}}}
...
["terrain"] = {type='function', description = " returns or sets scenenode as terrain. Children will be put on terrain with local matrix Z as offset in height.", args="(scenenode,[terrain]/[l3dterrain])", returns="(terrain)"},
...
["getnextname"] = {type='function', description = " returns a node within same namegroup of given node", args="(scenenode s3d)", returns="(scenenode s3d)"},
["get"] = {type='function', description = " returns a node", args="(string name)", returns="([scenenode s3d])"}}}
...
simpleshape = {type='class',description="shapes for the simple collision system",childs={}}
...
["l3dtargetoff"] = {type='function', description = " sets l3dtarget off", args="(l2dnode node)", returns="()"},
["l3dtarget"] = {type='function', description = " returns or sets l3dtarget", args="(l2dnode node,[l3dnode])", returns="([l3dnode])"},
...
["get"] = {type='function', description = " returns l2dnode with given name", args="(string)", returns="([l2dnode])"}}}
...
["manualprojection"] = {type='function', description = " returns or sets if manual projection matrix is used. That means aspect/frontplane.. will not modify projection matrix (settable via projmatrix).", args="(l3dcamera,[boolean])", returns="([boolean])"},
...
["new"] = {type='function', description = " creates new camera. The id must be unique and a l3dnode must get an update to its visflag, if it should be visible to it.", args="(int id 1-31)", returns="([l3dcamera])"},
...
["toworld"] = {type='function', description = " computes screen pixel position (reference coordinates) to world coordinates. The camera returns the coordinates seen from 0,0,0. You will need to transform the coordinate with a matrix, if you link the camera against an actornode/scenenode. If l3dview is not passed, we will use window.refsize.", args="(l3dcamera,float x,y,z,[l3dview])", returns="(float x,y,z)"},
...
["side"] = {type='function', description = " returns or sets side of texture (only for cubemaps) 0..5", args="(rcmdcopytex,[int])", returns="([int)"},
...
",childs={["namenext"] = {type='function', description = " returns the next actor of same name. Because it is                                 a ringlist, you have to take care about your starting point of the traversion.", args="()", returns="(actornode actor)"},
...
["vel"] = {type='function', description = " sets/gets velocity", args="(actornode self, [float x,y,z])", returns="([float x,y,z])"},
...
["lookat"] = {type='function', description = " rotates actornode to look at a point along defined axis. axis can be 0:x, 1:y (defualt) 2:z.", args="(actornode self, float tox,toy,toz,upx,upy,upz,[int axis 0-2])", returns="()"},
["name"] = {type='value', description = "[string name] name (ActorNode self, [string name]) - sets or gets the name"},
...
["new"] = {type='function', description = " creates new l3dlevelmodel. Visibility cubes are generated with the given separations (>=1).", args="(string name,|l3dlayerid|,model,int separatorsx,y,z, [int minpolys])", returns="([l3dlevelmodel])"}}}
...
["stencil"] = {type='function', description = " returns or sets renderbuffer attachment for stencil target. You can disable assignment by passing 0.", args="(rcmdfbotex,[texture])", returns="([texture])"},
...
["color"] = {type='function', description = " returns or sets renderbuffer attachments for color targets 0..3. You can disable certain assignments by passing 0. Special is the side of a cubemap, or the z slice of a 3dtexture.", args="(rcmdfbotex,int clr,[texture,[int special]])", returns="([texture,int special])"},
...
["get"] = {type='function', description = " returns node with that name", args="(string name)", returns="([l3dnode])"},
...
["gettrack"] = {type='function', description = " returns trackid of track within animation", args="(animation,string trackname)", returns="([trackid])"}}}
matobject = {type='class',description="Several classes which materials can be applied to, allow the detailed control of some material values. l3dmodels need to pass 2 arguments as matobject: l3dmodel,meshid. If you pass materials, then you will change values for the default matobject. If the node's material is changed, its old materialobject will be lost.",childs={["moPos"] = {type='function', description = " sets or returns position of texture matrix. Returns 0,0,0 if no matrix is set", args="(matobject,[float x,y,z])", returns="([float x,y,z])"},
...
["moTexmatrix"] = {type='function', description = " sets or returns texturematrix, pass 0 as matrix4x4 to disable.", args="(matobject,[matrix4x4])", returns="(matrix4x4)"},
...
["moTexcontrol"] = {type='function', description = " sets or returns material texcontrol value.", args="(matobject,matcontrolid,[texture])", returns="([texture])"},
...
["vistestdepth"] = {type='function', description = " sets/gets maximum octree depth for vistests.", args="([boolean])", returns="([boolean])"},
...
["drawtiles"] = {type='function', description = " draw tilesets", args="([boolean])", returns="([boolean])"},
["notextures"] = {type='function', description = " deactivate textures (only shows effect in debug build)", args="([boolean])", returns="([boolean])"},
...
["toString"] = {type='function', description = " returns a string code for the given\
		...
		", args="(object)", returns="(string)"}}}
texcombsrc = {type='class',description="The texcombiner source for the function specifies where values should be taken from.",childs={["texture3"] = {type='function', description = " texture at unit 3 (needs crossbar capability / higher technique than default)", args="()", returns="(texcombsrc)"},
...
tbterrain = {type='class',description="tiletexture-based terrain, uses elevation data and surface layers, which are organized as tiles.<br> Every tile has 4 layers of surfacetiles. You can use up to 256 surfacetiletypes, each with a unique texture. However on an area of 32x32 tiles no more than eight textures can be used.",childs={["savesurfname"] = {type='function', description = " returns or sets the savename for the surface layer (by default same as on set)", args="(tbterrain,int layer,[string])", returns="([string])"},
["tileheight"] = {type='function', description = " returns or sets tile height", args="(tbterrain,int x,int y,[float height])", returns="(TBTerrain terr)"},
["tilesurf"] = {type='function', description = " returns or sets tile surface id", args="(tbterrain,int x,int y,int layer,[int id])", returns="(TBTerrain terr)"},
["save"] = {type='function', description = " saves a terrain by storing the data into the savenames", args="(tbterrain)", returns="()"},
["tilecolor"] = {type='function', description = " returns or sets tile color", args="(tbterrain,int x,int y,[float r,g,b,a])", returns="(float r,g,b,a)"},
["savecolorname"] = {type='function', description = " returns or sets the savename for the colormap (by default same as on set)", args="(tbterrain,[string])", returns="([string])"},
["createfinish"] = {type='function', description = " initializes all needed data for rendering. Set all known tiletypes and layers before", args="(tbterrain)", returns="()"},
["createblank"] = {type='function', description = "		creates a new tbterrain. Dimensions must be >= 32.<br> After setting all used tiletypes and layers run the newfinish function.", args="(string name, int width,int height, float heightfactor)", returns="(tbterrain)"},
["size"] = {type='function', description = " returns width and height of terrain", args="(tbterrain)", returns="(int width,height)"},
["surfacetile"] = {type='function', description = " creates a new surface tile using the given id (1..255) and creates a special texture from the given texturename.", args="(tbterrain,int id, string tiletexturename)", returns="()"},
["saveobjname"] = {type='function', description = " returns or sets the savename for the object layer (by default same as on set)", args="(tbterrain,int layer,[string])", returns="([string])"},
["defaultpath"] = {type='function', description = " returns or sets the default resource path. Luxinia will search in those when resources are not found.", args="([string])", returns="([string])"},
["getrestype"] = {type='function', description = " returns the resource type as int value, useful for resdata or reschunk functions", args="()", returns="(int restype)"},
["createspecs"] = {type='function', description = " default specifications for newly generated terrains. Tilesize is the length of a single tile. Blocksize is the square grid that is visible. Camtolerance means that if camera moved more than this many tiles the block is updated. Atlascount is the number of surfacetiles that can be active within current visible block.<br><br>All values passed must be even, and atlascount should be power of 2.", args="([int tilesize, int blocksize, int camtolerance, int atlascount])", returns="([int tilesize, int blocksize, int camtolerance, int atlascount])"},
["surfacelayer"] = {type='function', description = " sets the given layer with the data from the data texture.", args="(tbterrain,int id, data texture)", returns="()"},
["tileupdate"] = {type='function', description = " updates tile, makes changes effective, optionally a whole region", args="(tbterrain,int x,int y,[int region])", returns="()"},
["saveheightname"] = {type='function', description = " returns or sets the savename for the heightmap (by default same as on set)", args="(tbterrain,[string])", returns="([string])"},
["height"] = {type='function', description = " returns interpolated height at x/y", args="(tbterrain self, float x,y)", returns="(float height)"},
["getcoords"] = {type='function', description = " converts world position into terrain coords", args="(tbterrain,float x,y,z)", returns="(int x,y)"},
["createfromtex"] = {type='function', description = " creates a new tbterrain using the given elevation and color data. Make sure the textures kept their data, and you can free the data afterwards. If the heightmap has 4 colorvalues it is used as float texture, else the red value is multiplied by heightfactor.<br> After setting all used tiletypes and layers run the createfinish function.", args="(string name, texture heightmap, float heightfactor, [texture colormap])", returns="(tbterrain)"},
["getheights"] = {type='function', description = " returns a mounted floatarray, in which you can directly access heights of the terrain grid. There is steps*2+1 floats for each dimension.", args="(tbterrain)", returns="(floatarray)"}}}
...
l3dtext = {type='class',description="A text node",childs={["font"] = {type='function', description = " returns or sets font texture", args="(l3dtext,[texture])", returns="([texture])"},
...
Compat = {type='class',description="Compat is a class that provides compatibility functions for previous \
		...
		automatically applied. Else load compatibility manually. Sometimes backwards compatibility cannot be achived.",childs={["setcompatibility"] = {type='function', description = " sets compatibility for the given version string e.g \"0.91f\" ", args="(string version)", returns="()"}}}
...
["getloaded"] = {type='function', description = " returns a table with all loaded resources of the given type.", args="(reschunk, int restype)", returns="(int count,[resource table])"},
...
["getboneparents"] = {type='function', description = " returns an array of the parent index for every bone. We start at 0 not 1. -1 means it has no parent. With model.boneid(index) command you can get more info about the bone.", args="(model)", returns="([table int])"},
...
["setmeshtype"] = {type='function', description = " updates all meshes with given type: 0 auto 1 display list, 2 vertex array, 3 vertexbufferobject. defaults: lmcoords = false, vertexcolors = true", args="(model,int type,[boolean lmcoords],[boolean vertexcolors])", returns="()"},
...
rcmddraw2dmesh = {type='class',description="Draws a 2D mesh, be aware that mesh winding is opposite (CCW) of l2dnodes (CW). Positions and sizes are always in OpenGL coordinates (0,0) = bottom left. The l2dnodes' reference size system is not used.",childs={["pos"] = {type='function', description = " returns or sets. Only used when autosize is 0. Coordinates in OpenGL (0,0) = bottomleft of current l3dview", args="(rcmddraw2dmesh,[float x,y,z])", returns="([float x,y,z])"},
...
["size"] = {type='function', description = " returns or sets. Only used when autosize is 0. Coordinates in OpenGL (0,0) = bottomleft of current l3dview", args="(rcmddraw2dmesh,[float x,y,z])", returns="([float x,y,z])"},
...
["vertex32texcoords"] = {type='function', description = " size 32 bytes, position (3 floats), color (4 unsigned byte), 2 texcoord channels (2*2 floats).", args="()", returns="(vertextype)"},
...
["vertex64"] = {type='function', description = " size 64 bytes, normals (3 floats), position (3 floats), color (4 unsigned bytes), 4 texcoord channels (4*2 floats). The last 2 texcoord channels serve as user4 (4 floats) which may be used for hardware skinning.", args="()", returns="(vertextype)"}}}
indexarray = {type='class',description="meshes contain indices for rendering their primitives, with this interface you can modify them. Always make sure that the corresponding vertices exist and always run indexMinmax once you set all indices. If you use display lists or vbos, changes to a loaded model will not be visible.",childs={["indexTrianglecount"] = {type='function', description = " returns and updates the internal number of triangles based on current indexCount. This is only needed for statistics and should be called at least once after all indices were written.", args="(indexarray)", returns="([int])"},
...
["indexUpdateVBO"] = {type='function', description = " if possible resubmits array using indexAllocatedcount to VBO, returns true on success else false (no VBO or non compatible indexarray).", args="(indexarray)", returns="(boolean)"},
...
["indexMinmax"] = {type='function', description = " computes minimum and maximum index used within indexCount, or you can manually set them. This is required for an indexarray to work, call this after all indices are set.", args="(indexarray,[min,max])", returns="(int min,max)"},
...
["indexMeshtype"] = {type='function', description = " returns what type of mesh the indices/vertices are stored in (unset,va,vbo) ", args="(indexarray)", returns="([string])"},
...
["indexPtrs"] = {type='function', description = " returns C pointers for begin and end of array. [start,end[", args="(indexarray)", returns="(int start,end])"}}}
...
["fdirpriority"] = {type='function', description = " sets/gets priority of the fdirnormal. The priority can be used in different ways.", args="(dgeom geom,[float])", returns="([float])"},
...
input = {type='class',description="Luxinia's function publishing  does not offer direct calling of external functions if an event arises, such as key or mouseinputs. Instead, Luxinia saves event occurances in queues that can be processed when the scripting language is running.",childs={["joystick"] = {type='function', description = " returns joystick position and currently pressed buttons or false, if the given number does not exist. If no number is passed, joystick 0 is used. nr must be smaller than 16 and >= 0. Luxinia limits the number of axis to 8 and the number of buttons to 32, which should be enough.", args="([int nr])", returns="(boolean exists,[int joysticknr,int axiscount,int btncnt, float pos1,..., boolean btn1,...])"},
...
["popkey"] = {type='function', description = " pops a key from keyhistoryqueue. The key is the resulted char of the keyboard. The keycode is the number on the keyboard, which doesn't have to be a printable letter. In case of unprintable letters, key is 0.", args="()", returns="([int key, int keycode, boolean isDown])"},
["mousewheel"] = {type='function', description = " sets or gets mousewheelposition", args="([int])", returns="([int])"}}}
staticarray = {type='class',description="Staticarrays are created as plain C arrays within luxinia and cannot be resized. They can be used for operations that require lot's of calculations or are used by other C functions.",childs={["clamp"] = {type='function', description = " self = (self with values clamped to min,max)", args="(staticarray self, value min, value max)", returns="()"},
["index"] = {type='function', description = " assigns index the float value if given or simply returns the value at the index. Returns error in case of invalid indices.", args="(staticarray,index,[value])", returns="([value])"},
...
["count"] = {type='function', description = " returns maximum number of floatelements", args="(staticarray)", returns="(int)"},
...
["datapointer"] = {type='function', description = " you can access the memory directly in other lua dlls. Be aware that you must make sure to not corrupt memory, make sure you are always smaller than the 'end' pointer.", args="(staticarray self)", returns="(int start, int end)"}}}
