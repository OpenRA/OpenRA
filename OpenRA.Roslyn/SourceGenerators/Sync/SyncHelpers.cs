...
namespace OpenRA.Roslyn.SourceGenerators.Sync
{
	static class SyncHelpers
	{
		...
		public static bool ImplementsISync(this INamedTypeSymbol type)
			=> type.AllInterfaces.Any(y => y.ToDisplayString() == SyncInterfaceName);

		public static bool ManuallyImplementsISync(this INamedTypeSymbol type)
			=> type.ImplementsISync() && type.MemberNames.Contains(SyncHashMethodName);
		...
		public static bool HasVerifySyncAttribute(this ISymbol symbol)
			=> symbol.GetAttributes().Any(x => x.AttributeClass.Name == VerifySyncAttributeName);
	}
}
