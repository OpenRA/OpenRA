...
SneakPath = { waypoint7.Location, waypoint9.Location, waypoint2.Location, CPos.New(60,38), waypoint4.Location }
...
InitAI = function()
	...
	RepairNamedActors(EnemyAi, 0.75)
	AiProcsNumber = 2
	if ProcUpgrade then
		ProcUpg = Actor.Create(ProcUpgrade, true, { Owner = EnemyAi })
	end
	AiAnyhqPrerequisite = Actor.Create("AiAnyhqPrerequisite", true, { Owner = EnemyAi })
	AiTmplPrerequisite = Actor.Create("AiTmplPrerequisite", true, { Owner = EnemyAi })
...
ApacheBackdoor = function()
	if NodOutPostHpad.IsDead or NodOutPostHpad.Owner ~= EnemyAi then
		return
	end
...
ApacheGuard = function(apache, hpad, topleft, botright, init)
	...
	local targets = Map.ActorsInBox(Map.CenterOfCell(topleft), Map.CenterOfCell(botright), function(a) return a.Owner == Player1 and a.Type ~= "camera.small" end)
	if #targets > 0 then
		apache.Hunt()
	else
...
ApacheRebuild = function(hpad, topleft, botright)
	if hpad.IsDead and hpad.Owner == EnemyAi then
		return
	end
...
GuardBuilding = function(building)
	Trigger.OnDamaged(building, function(_, atk, _)
		if atk.Type ~= "player" and not atk.IsDead and atk.Owner == Player1 then
...
CheckBuildablePlace = function(b)
	local blockingActors = Map.ActorsInBox(WPos.New(b.pos.X * 1024, b.pos.Y * 1024, 0), WPos.New((b.pos.X + BuildingSizes[b.type].X) * 1024, (b.pos.Y + BuildingSizes[b.type].Y) * 1024, 0), function(a) return a.Owner == Player1 end)
...
CheckBase = function()
	...
	if Utils.All(NodCYards, function(cy) return cy.IsDead or cy.Owner ~= EnemyAi end) then
		return
	end

	local queuedProcs = 0
	for i = 1, #NodRebuildList do
		if queuedProcs >= AiProcsNumber and EnemyAi.Resources < 4000 then
			break
		...
		elseif EnemyAi.PowerProvided - EnemyAi.PowerDrained + NodRebuildList[i].power > 0 or Utils.Any(PowerTypes, function(pt) return pt == NodRebuildList[i].type end) or NodRebuildList[i].type == "proc" then
...
BuildBuilding = function(building, cyards)
	if CyardIsBuilding or EnemyAi.Resources < Actor.Cost(building.type) then
	...
	EnemyAi.Resources = EnemyAi.Resources - Actor.Cost(building.type)

	Trigger.AfterDelay(Actor.BuildTime(building.type), function()
		CyardIsBuilding = false

		if Utils.All(cyards, function(cy) return cy.IsDead or cy.Owner ~= EnemyAi end) then
			EnemyAi.Resources = EnemyAi.Resources + Actor.Cost(building.type)
			return
		end
		...
		local newBuilding = Actor.Create(building.type, true, { Owner = EnemyAi, Location = building.pos })
		for i = 1, #NodRebuildList do
		...
		RepairBuilding(EnemyAi, newBuilding, 0.75)
...
CheckProduction = function()
	if Utils.All(ProductionBuildings, function(b) return b.IsDead end) then
		return
	elseif #EnemyAi.GetActorsByType("proc") < 1 and EnemyAi.Resources < 6000 then
		RestartUnitProduction()
		return
	...
	if ProductionCooldown and #UniqueTeamsQueue < 1 or EnemyAi.Resources < 4000 then
		RestartUnitProduction()
		return false
	else
		ProduceUnits()
...
CreateUnitsGroup = function()
	...
	Utils.Do(Utils.Random(team), function(u)
		if u == "vehicle" or u == "aircraft" then
			pb = u
		elseif ProductionBuildings[pb] and not ProductionBuildings[pb].IsDead and ProductionBuildings[pb].Owner == EnemyAi then
...
CheckForHarvester = function()
	local harv = EnemyAi.GetActorsByType("harv")
