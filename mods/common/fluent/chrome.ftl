...
label-window-resolution-container-size = Output Resolution:
...
label-renderer-performance-preset = Rendering performance preset (sets controls below): { $preset }
options-renderer-performance =
    .quality = Quality
    .balanced = Balanced
    .fast = Fast
    .very-fast = Very Fast
    .minimum = Minimum CPU
    .custom = Custom
label-world-render-scale = Internal battlefield resolution: 1/{ $scale } ({ $width }x{ $height })
label-frame-render-scale = Whole-frame resolution: 1/{ $scale } ({ $width }x{ $height })
checkbox-world-render-nearest =
    .label = Fast nearest-neighbor upscale
    .tooltip = Use a cheap, crisp upscale instead of the filtered pixel-art scaler.
checkbox-world-render-simple-scaling =
    .label = Fast simple scaler
    .tooltip = Skip the expensive pixel-art shader. Linear scaling may look slightly softer.
checkbox-direct-render =
    .label = Direct display rendering
    .tooltip = Skip the full-window compositor buffer at 1/1 whole-frame resolution. Disable only for driver compatibility.
checkbox-world-render-native-blit =
    .label = Native low-quality upscale
    .tooltip = Let the driver copy the battlefield directly. Fastest with software rendering, but scrolling snaps to internal pixels.
checkbox-world-render-shadows =
    .label = Decorative shadows
    .tooltip = Draw sprite and actor shadows. Disable this to reduce render preparation work.
checkbox-world-render-post-processing =
    .label = Post-processing effects
    .tooltip = Draw battlefield post-processing effects. Disable this to save fill rate and draw calls.
checkbox-world-render-antialiasing =
    .label = Smooth range overlays
    .tooltip = Antialias range circles and other battlefield overlays. Disable this for the cheapest rendering path.
checkbox-prioritize-game-tick =
    .label = Prioritize simulation when slow
    .tooltip = Let rendering skip a late game tick instead of slowing the simulation and controls.
