...
IxianReinforcementsHaveArrived = UserInterface.String("ixian-reinforcements-arrived")
...
Tick = function()
	if not Ordos.IsObjectiveCompleted(KillAtreides) and Atreides.HasNoRequiredUnits() then
		Media.DisplayMessage(UserInterface.String("atreides-annihilated"), Mentat)
	...
	if TimerTicks and TimerTicks > 0 then
		...
		if TimerTicks == 0 then
			if not FirstIxiansArrived then
				Media.DisplayMessage(UserInterface.String("deliveries-arriving-massive-reinforcements"), Mentat)
			end
		...
		elseif (TimerTicks % DateTime.Seconds(1)) == 0 then
			...
			local reinforcementsText = UserInterface.String("initial-reinforcements-arrive-in", { ["time"] = time })
			if FirstIxiansArrived then
				reinforcementsText = UserInterface.String("additional-reinforcements-arrive-in", { ["time"] = time })
			end
...
WorldLoaded = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(2), function()
		...
		local time = Utils.FormatTime(TimerTicks)
		Media.DisplayMessage(UserInterface.String("ixian-reinforcements-in", { ["time"] = time }), Mentat)
	end)
	...
	local announcementFunction = function()
		Media.DisplayMessage(UserInterface.String("enemy-reinforcements-arrived"), Mentat)
	end
