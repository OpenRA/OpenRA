...
SendStarportReinforcements = function()
	Trigger.AfterDelay(CorrinoStarportDelay[Difficulty], function()
		...
		Media.DisplayMessage(UserInterface.String("imperial-ships-penetrating-defense-grid"), Mentat)
	end)
end
...
OrdosReinforcementNotification = function(currentWave, totalWaves)
	Trigger.AfterDelay(OrdosAttackDelay[Difficulty], function()
		...
		Media.DisplayMessage(UserInterface.String("enemy-carryall-drop-detected"), Mentat)
...
Tick = function()
	...
	if Corrino.HasNoRequiredUnits() and not Harkonnen.IsObjectiveCompleted(KillCorrino) then
		Media.DisplayMessage(UserInterface.String("emperor-annihilated"), Mentat)
...
WorldLoaded = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(5), function()
		Media.DisplayMessage(UserInterface.String("protect-outpost"), Mentat)
	end)
