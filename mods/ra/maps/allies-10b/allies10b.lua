...
DeactivateMissiles = function()
	...
	Trigger.OnEnteredFootprint(TopLeftConsole, function(actor, id)
		if actor.Type == "e6" and not TopLeftTriggered then
			...
			Media.DisplayMessage(UserInterface.String("nuclear-missile-deactivated"), UserInterface.String("console"))
		end
	end)
...
Tick = function()
	...
	if Ticked > 0 then
		if Ticked % DateTime.Seconds(1) == 0 then
			local timer = UserInterface.String("reach-target-in", { ["time"] = Utils.FormatTime(Ticked) })
			UserInterface.SetMissionText(timer, USSR.Color)
		end
		Ticked = Ticked - 1
	elseif Ticked == 0 then
		FinishedTimer = UserInterface.String("we-are-too-late")
