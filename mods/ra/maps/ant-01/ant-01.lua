...
AlliedForcesHaveArrived = UserInterface.String("allied-forces-have-arrived")
	...
	if SurviveObjective ~= nil then
		if Ticks > 0 then
			...
			if (Ticks % DateTime.Seconds(1)) == 0 then
				Timer = UserInterface.String("reinforcements-arrive-in", { ["time"] = Utils.FormatTime(Ticks) })
