...
Tick = function()
	...
	if Allies2.IsLocalPlayer and DateTime.GameTime <= ReinforcementsDelay then
		if DateTime.GameTime % DateTime.Seconds(1) == 0 then
			...
			local timer = UserInterface.String("allied-reinforcements-arrive-in", { ["time"] = time })
...
SpawnTanya = function()
	...
	if Difficulty ~= "easy" and Allies1.IsLocalPlayer then
		Trigger.AfterDelay(DateTime.Seconds(2), function()
			Media.DisplayMessageToPlayer(Allies1, UserInterface.String("tanya-rules-of-engagement"), UserInterface.String("tanya"))
		end)
	end
end
...
WorldLoaded = function()
	...
	local dontLoseMoreThan = UserInterface.String("do-not-lose-more-than", { ["units"] = DeathThreshold[Difficulty] })
