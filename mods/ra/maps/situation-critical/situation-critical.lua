...
SetupTriggers = function()
	...
	Trigger.OnInfiltrated(BioLab, function()
		Media.DisplayMessage(UserInterface.String("plans-stolen-erase-data"), UserInterface.String("scientist"))
...
SendInVolkov = function()
	if not VolkovArrived then
		...
		Trigger.OnAddedToWorld(teamVolkov[1], function(a)
			Media.DisplayMessage(UserInterface.String("software-update-failed-manual-targets"), UserInterface.String("volkov"))
		end)
...
Tick = function()
	...
	if Ticked > 0 then
		if (Ticked % DateTime.Seconds(1)) == 0 then
			Timer = UserInterface.String("missiles-launch-in", { ["time"] = Utils.FormatTime(Ticked) })
