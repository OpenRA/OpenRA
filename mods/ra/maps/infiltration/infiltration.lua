...
SecureLabTimer = function()
	...
	if Ticked > 0 then
		if (Ticked % DateTime.Seconds(1)) == 0 then
			Timer = UserInterface.String("secure-lab-in", { ["time"] = Utils.FormatTime(Ticked) })
			UserInterface.SetMissionText(Timer, TimerColor)
		end
		Ticked = Ticked - 1
	elseif Ticked <= 0 then
		TimerColor = Soviets.Color
		UserInterface.SetMissionText(UserInterface.String("soviet-research-lab-not-secured-in-time"), TimerColor)
		SecureLabFailed()
	end
end
