...
RunForIt = function()
	...
	Media.DisplayMessage(UserInterface.String("run-for-it"), UserInterface.String("convoy-commander"))
	Utils.Do(Trucks, function(truck)
...
WeAreSurrounded = UserInterface.String("we-are-surrounded")
	...
	if StartTimer then
		if Ticked > 0 then
			if (Ticked % DateTime.Seconds(1)) == 0 then
				Timer = UserInterface.String("corridor-closes-in", { ["time"] = Utils.FormatTime(Ticked) })
