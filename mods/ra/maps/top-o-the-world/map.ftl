## rules.yaml
actor-powerproxy-spyplane =
    .airstrikepower-name = Spy Plane (Single Use)
    .airstrikepower-description = Reveals an area of the map and uncovers cloaked enemy units.

## rules.yaml
briefing = 
    A critical cargo held in this supply truck must reach the waystation atop the mountain. The way is blocked by Allied squads, but do not let that deter you -- the cargo must reach its destination.
    
    The onset of nightfall has brought sub-zero temperatures with it. The Allies are prepared for these conditions -- our units are not. The life expectancy of your troops in these extremes is limited. Infantry may last 12 minutes, while vehicles could last for up to 36 minutes. You have little time. Go.
