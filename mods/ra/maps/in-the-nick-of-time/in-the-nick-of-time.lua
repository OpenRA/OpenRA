			...
			if not USSRHouse.IsDead then
				...
				Trigger.OnAllKilled(houseSquad, function()
					if not AlliesHouse.IsDead then
						...
						Media.DisplayMessage(UserInterface.String("friendlies-coming-out"), UserInterface.String("medic"))
	...
	Trigger.OnEnteredProximityTrigger(ScientistEscape.CenterPosition, WDist.FromCells(3), function(actor, id)
		if actor.Owner == Greece then
			...
			Trigger.AfterDelay(DateTime.Seconds(5), function()
				Media.PlaySpeechNotification(Greece, "ReinforcementsArrived")
				Media.DisplayMessage(UserInterface.String("get-scientists-evacuation-point"), UserInterface.String("landcom-16-capitalized"))
...
LoseTriggers = function()
	Trigger.OnKilled(ForwardCommand, function()
		if not ScientistsFreed then
			Greece.MarkFailedObjective(RescueScientists)
			Media.DisplayMessage(UserInterface.String("scientists-killed-in-command-center"), UserInterface.String("landcom-16-capitalized"))
		end
	end)
...
Tick = function()
	...
	if Ticked > 0 then
		if (Ticked % DateTime.Seconds(1)) == 0 then
			Timer = UserInterface.String("chronosphere-explodes-in", { ["time"] = Utils.FormatTime(Ticked) })
			UserInterface.SetMissionText(Timer, TimerColor)
		end
		Ticked = Ticked - 1
	elseif Ticked == 0 then
		UserInterface.SetMissionText(UserInterface.String("too-late"), USSR.Color)
		USSR.MarkCompletedObjective(SovietObj)
	end
end
