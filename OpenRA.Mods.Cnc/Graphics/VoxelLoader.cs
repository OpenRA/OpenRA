...
{
	public sealed class VoxelLoader : IModelLoader
	{
		...
		public bool TryLoadModel(IReadOnlyFileSystem fileSystem, string filename, MiniYaml yaml, out IModel model)
		{
			...
			model = new VoxelModel(this, vxlReader, hvaReader, (vxl, hva));
		...
		public ModelRenderData GenerateRenderData(VxlLimb l)
		{
			...
			return new ModelRenderData(start, count, Game.Renderer.GetShader<ModelShaderBindings>(), VertexBuffer,
				new Dictionary<string, ITexture>() { { "DiffuseTexture", sheetBuilder.Current.GetTexture() } });
		}
