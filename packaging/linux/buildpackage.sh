...
(
    ...
    cd deb
    ./buildpackage.sh "$TAG" ../$ROOTDIR "$PACKAGEDIR" &> $PWD/package.log
    if [ $? -ne 0 ]; then
...
(
    ...
    cd pkgbuild
    sh buildpackage.sh "$TAG" ../$ROOTDIR "$PACKAGEDIR" &> $PWD/package.log
    if [ $? -ne 0 ]; then
...
(
    ...
    cd rpm
    sh buildpackage.sh "$TAG" ../$ROOTDIR ~/rpmbuild "$PACKAGEDIR" &> $PWD/package.log
    if [ $? -ne 0 ]; then
