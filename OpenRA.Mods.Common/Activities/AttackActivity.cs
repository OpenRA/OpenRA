	...
	class AttackActivity : Activity, IActivityNotifyStanceChanged
	{
		...
		public override bool Tick(Actor self)
		{
			...
			if (!targetIsHiddenActor && target.Type == TargetType.Actor)
			{
				...
				if (canMove && target.Actor.Info.HasTraitInfo<IMoveInfo>())
				{
			...
			if (checkTarget.IsInRange(pos, maxRange) && !checkTarget.IsInRange(pos, minRange))
			{
				...
				if (!attack.TargetInFiringArc(self, target, attack.Info.FacingTolerance))
				{
					...
					var mobile = (Mobile)move;
					mobile.Facing = Util.TickFacing(mobile.Facing, desiredFacing, mobile.TurnSpeed);
					return false;
				}
			...
			if (!canMove || maxRange == WDist.Zero || maxRange < minRange)
				return true;
