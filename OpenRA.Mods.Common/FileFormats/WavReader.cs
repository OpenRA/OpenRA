		...
		public sealed class WavStreamMsAdpcm : ReadOnlyAdapterStream
		{
			...
			protected override bool BufferData(Stream baseStream, Queue<byte> data)
			{
				...
				for (var blockindx = 0; blockindx < blockDataSize; blockindx++)
				{
					...
					WriteSample(DecodeNibble((short)((bytecode >> 4) & 0x0F), bpred[0], ref chan_idelta[0], ref s1[0], ref s2[0]), data);

					// Decode the second nibble, for stereo this will be the right channel
					WriteSample(DecodeNibble((short)(bytecode & 0x0F), bpred[chanr], ref chan_idelta[chanr], ref s1[chanr], ref s2[chanr]), data);
				}
			...
			private short DecodeNibble(short nibble, byte bpred, ref short idelta, ref short s1, ref short s2)
			{
				...
				s2 = s1;
				s1 = ClampInt16((twosCompliment * idelta) + predict);
