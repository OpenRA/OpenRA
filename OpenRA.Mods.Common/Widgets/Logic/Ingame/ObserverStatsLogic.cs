...
namespace OpenRA.Mods.Common.Widgets.Logic
{
	public class ObserverStatsLogic
	{
		...
		ScrollItemWidget EconomyStats(Player player)
		{
			...
			harvesters.GetText = () => world.Actors.Count(a => a.Owner == player && !a.IsDead && a.Info.TraitInfosAny<HarvesterInfo>()).ToString();
		...
		ScrollItemWidget SetupPlayerScrollItemWidget(ScrollItemWidget template, Player player)
		{
			...
			{
				var playerBase = world.Actors.FirstOrDefault(a => !a.IsDead && a.Info.TraitInfosAny<BaseBuildingInfo>() && a.Owner == player);
				if (playerBase != null)
