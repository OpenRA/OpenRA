					...
					if (!fetchedNews)
					{
						Task.Run(async () =>
						{
							try
							{
								...
								var url = new QueryBuilder(webServices.GameNews)
								{
								...
								Game.RunAfterTick(() => // run on the main thread
								{
									...
									if (currentNews == null || newNews.Any(n =>
										!currentNews.Select(c => c.DateTime).Contains(n.DateTime)))
