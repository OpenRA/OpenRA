...
namespace OpenRA.Mods.Common.Widgets
{
	...
	public class ProductionTabsWidget : Widget
	{
		...
		readonly Dictionary<ProductionQueue, bool> cachedProductionQueueEnabledStates = new();
		...
		public void ActorChanged(Actor a)
		{
			if (a.Info.HasTraitInfo<ProductionQueueInfo>())
			{
				...
				foreach (var queue in queues)
					cachedProductionQueueEnabledStates[queue] = queue.Enabled;

				foreach (var g in Groups.Values)
					g.Update(cachedProductionQueueEnabledStates.Keys);
		...
		public override void Tick()
		{
			...
			foreach (var pair in cachedProductionQueueEnabledStates)
			{
				if (pair.Key.Enabled != pair.Value)
				{
