...
namespace OpenRA.Mods.Common.Pathfinder
{
	public sealed class PathSearch : BasePathSearch
	{
		...
		public static IPathSearch Search(World world, LocomotorInfo li, Locomotor locomotor, Actor self, bool checkForBlocked, Func<CPos, bool> goalCondition)
		{
			var graph = new PathGraph(LayerPoolForWorld(world), li, locomotor, self, world, checkForBlocked);
		...
		public static IPathSearch FromPoint(World world, LocomotorInfo li, Locomotor locomotor, Actor self, CPos @from,
			CPos target, bool checkForBlocked)
		{
			var graph = new PathGraph(LayerPoolForWorld(world), li, locomotor, self, world, checkForBlocked);
			var search = new PathSearch(graph)
			{
		...
		public static IPathSearch FromPoints(World world, LocomotorInfo li, Locomotor locomotor, Actor self, IEnumerable<CPos> froms, CPos target, bool checkForBlocked)
		{
			var graph = new PathGraph(LayerPoolForWorld(world), li, locomotor, self, world, checkForBlocked);
			var search = new PathSearch(graph)
			{
