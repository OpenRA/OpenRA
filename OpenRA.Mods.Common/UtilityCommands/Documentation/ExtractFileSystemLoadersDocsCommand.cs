...
{
	sealed class ExtractFileSystemLoadersDocsCommand : IUtilityCommand
	{
		string IUtilityCommand.Name => "--filesystem-loaders-docs";

		bool IUtilityCommand.ValidateArguments(string[] args) => true;

		[Desc("[VERSION]", "Generate file system loaders documentation in JSON format.")]
		void IUtilityCommand.Run(Utility utility, string[] args)
		{
			...
			var fileSystemLoaders = objectCreator.GetTypesImplementing<IFileSystemLoader>().OrderBy(t => t.Namespace).ThenBy(t => t.Name);

			var json = GenerateJson(version, fileSystemLoaders, objectCreator);
			Console.WriteLine(json);
		}

		static string GenerateJson(string version, IEnumerable<Type> fileSystemLoaderTypes, ObjectCreator objectCreator)
		{
			var relatedEnumTypes = new HashSet<Type>();
			var pdbReaderCache = Utilities.CreatePdbReaderCache();

			var fileSystemLoaders = fileSystemLoaderTypes
			...
			{
				Version = version,
				FileSystemLoaders = fileSystemLoaders,
