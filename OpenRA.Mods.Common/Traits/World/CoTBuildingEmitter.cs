...
namespace OpenRA.Mods.Common.Traits
{
	...
	public sealed class CoTBuildingEmitter : PausableConditionalTrait<CoTBuildingEmitterInfo>, INotifyAddedToWorld, INotifyDamageStateChanged, INotifyKilled, ITick
	{
		...
		void SendEvent(Actor self, double lat, double lon, string type, string callsign, int staleSeconds, string reason, string milsymOverride = null)
		{
			...
			var milsymId = !string.IsNullOrEmpty(milsymOverride) ? milsymOverride : ResolveMilsymId(self);
		...
		void INotifyAddedToWorld.AddedToWorld(Actor self)
		{
			...
			if (createdByPlacement && !placedNotified && !IsTraitDisabled && !IsTraitPaused)
			{
				...
				if (router != null)
				{
					if (router.ShouldEmit(self, CoTDomain.Building, out var overrideType, out var overrideMilsym))
					{
						var type = string.IsNullOrEmpty(overrideType) ? ResolveType(self) : overrideType;
						SendEvent(self, lat, lon, type, ResolveCallsign(self), info.StaleSeconds, "placed", overrideMilsym);
						wasEmitting = true;
					}
					else
						wasEmitting = false;
		...
		void INotifyDamageStateChanged.DamageStateChanged(Actor self, AttackInfo e)
		{
			...
			if (router != null)
			{
				...
				{
					var type = string.IsNullOrEmpty(overrideType) ? ResolveType(self) : overrideType;
					SendEvent(self, lat, lon, type, ResolveCallsign(self), info.StaleSeconds, "damage", overrideMilsym);
					wasEmitting = true;
				}
			}
			else
		...
		void INotifyKilled.Killed(Actor self, AttackInfo e)
		{
			...
			if (router != null)
			{
				...
				{
					var type = string.IsNullOrEmpty(overrideType) ? ResolveType(self) : overrideType;
					SendEvent(self, lat, lon, type, ResolveCallsign(self), info.StaleSeconds, "killed", overrideMilsym);
				}
			}
			else
		...
		void ITick.Tick(Actor self)
		{
			...
			if (router != null)
			{
				...
				{
					var type = string.IsNullOrEmpty(overrideType) ? ResolveType(self) : overrideType;
					SendEvent(self, lat, lon, type, ResolveCallsign(self), info.StaleSeconds, moved ? "heartbeat+move" : "heartbeat", overrideMilsym);
					wasEmitting = true;
				}
				else
				{
