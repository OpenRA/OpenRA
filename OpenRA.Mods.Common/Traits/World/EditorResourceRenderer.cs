...
{
	[Desc("Visualizes the state of the `EditorResourceLayer`.", " Attach this to the world actor.")]
	public class EditorResourceRendererInfo : TraitInfo, Requires<EditorResourceLayerInfo>
	{
		[FieldLoader.Require]
		[Desc("Only render these ResourceType names.")]
		public readonly string[] RenderTypes = null;

		public override object Create(ActorInitializer init) { return new EditorResourceRenderer(init.Self, this); }
	}

	public class EditorResourceRenderer : IWorldLoaded, IRenderOverlay, ITickRender, INotifyActorDisposing
	{
		protected readonly EditorResourceLayer ResourceLayer;
		protected readonly CellLayer<RendererCellContents> RenderContent;
		protected readonly EditorResourceRendererInfo Info;
		...
		public EditorResourceRenderer(Actor self, EditorResourceRendererInfo info)
		{
			Info = info;

			ResourceLayer = self.Trait<EditorResourceLayer>();
