...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class ResourceRenderer : IWorldLoaded, IRenderOverlay, ITickRender, INotifyActorDisposing
	{
		...
		protected readonly CellLayer<CellRenderContents> RenderContent;
		...
		public ResourceRenderer(Actor self, ResourceRendererInfo info)
		{
			...
			RenderContent = new CellLayer<CellRenderContents>(self.World.Map);
		...
		void IWorldLoaded.WorldLoaded(World w, WorldRenderer wr)
		{
			...
			foreach (var cell in w.Map.AllCells)
			{
				...
				if (type != null && Info.RenderTypes.Contains(type.Info.Type))
				{
					...
					var cellContent = CreateResourceCell(type);
		...
		void ITickRender.TickRender(WorldRenderer wr, Actor self)
		{
			...
			foreach (var c in dirty)
			{
				if (!self.World.FogObscures(c))
				{
					...
					if (t != null)
					{
						var cellContent = CreateResourceCell(t);
		...
		CellRenderContents CreateResourceCell(ResourceType t)
		{
			return new CellRenderContents
			{
				Type = t,
		...
		public struct CellRenderContents
		{
