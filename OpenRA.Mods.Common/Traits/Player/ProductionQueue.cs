...
namespace OpenRA.Mods.Common.Traits
{
	...
	public class ProductionQueue : IResolveOrder, ITick, ITechTreeElement, INotifyOwnerChanged, INotifyKilled, INotifySold, ISync, INotifyTransform, INotifyCreated
	{
		...
		public virtual void ResolveOrder(Actor self, Order order)
		{
			...
			switch (order.OrderString)
			{
				case "StartProduction":
					StartProduction(self, order.TargetString, order.ExtraData, !order.Queued);
					break;
		...
		protected virtual void StartProduction(Actor self, string itemName, uint numberToProduce, bool hasPriority = false)
		{
		...
		protected bool CancelProductionInner(string itemName)
		{
			...
			if (item != null)
			{
				...
				{
					item.Infinite = false;
					StartProduction(Actor, item.Item, (uint)Info.InfiniteBuildLimit - 1);
				}
				else
				{
