...
namespace OpenRA.Mods.Common.Traits
{
	...
	[Desc("Resource map for Adaptive AI with fair fog-of-war enemy tracking.")]
	public class AdaptiveResourceMapBotModuleInfo : ConditionalTraitInfo, NotBefore<IResourceLayerInfo>
	{
		...
		public override object Create(ActorInitializer init) { return new AdaptiveResourceMapBotModule(init.Self, this); }
	}

	public class AdaptiveResourceMapBotModule : IBotTick
	{
		readonly World world;
		readonly Player player;
		IResourceLayer resourceLayer;
		public readonly AdaptiveResourceMapBotModuleInfo Info;
		...
		public AdaptiveResourceMapBotModule(Actor self, AdaptiveResourceMapBotModuleInfo info)
		{
			world = self.World;
			player = self.Owner;
			...
			var resourceCreatorLocs = world.FindActorsInCircle(world.Map.CenterOfCell(indice.IndiceCenter), WDist.FromCells(indiceResourceScanRadius))
				.Where(a =>
				{
					if (a.Owner.RelationshipWith(player) == PlayerRelationship.Enemy
						&& AdaptiveAIUtils.IsVisibleEnemy(a, player))
					{
