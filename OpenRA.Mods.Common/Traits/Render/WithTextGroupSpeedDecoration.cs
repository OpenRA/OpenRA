...
{
	[Desc("Renders group speedlimits using typeface.")]
	public class WithTextGroupSpeedDecorationInfo : ITraitInfo, IRulesetLoaded, Requires<IMoveInfo>
	{
		...
		public readonly ReferencePoints ReferencePoint = ReferencePoints.Bottom | ReferencePoints.Right;

		[Desc("Manual offset in screen pixel.")]
		public readonly int2 ScreenOffset = new int2(2, -2);

		[Desc("Show the numerical speed limit. Otherwise an S is used.")]
		public readonly bool DisplayValue = false;
		...
		public object Create(ActorInitializer init) { return new WithTextGroupSpeedDecoration(init.Self, this); }
	}

	public class WithTextGroupSpeedDecoration : IRenderAboveShroudWhenSelected, INotifyCapture, INotifyCreated
	{
		readonly WithTextGroupSpeedDecorationInfo info;
		readonly SpriteFont font;

		Color color;
		IMove move;

		public WithTextGroupSpeedDecoration(Actor self, WithTextGroupSpeedDecorationInfo info)
		{
			this.info = info;
		...
		{
			var speedLimit = move.SpeedLimit;
			if (speedLimit == 0)
				yield break;

			var bounds = self.VisualBounds;
			var display = info.DisplayValue ? speedLimit.ToString() : "s";
			var halfSize = font.Measure(display) / 2;
			...
			yield return new TextRenderable(font, wr.ProjectedPosition(screenPos), info.ZOffset, color, display);
		}
